...
const filesDB = {
  async find(filter = {}, sort = { pinned: -1, uploadDate: -1 }, limit = 500) {
          ...
          for (let key in cond) {
            ...
            if (cond[key] instanceof RegExp) {
              if (Array.isArray(val)) {
                if (val.some(v => cond[key].test(v))) return true;
              } else if (val && cond[key].test(String(val))) {
                return true;
              }
            }
          }
    ...
    if (filter.extension) {
      if (typeof filter.extension === 'string') {
        list = list.filter(f => (f.extension || '').toLowerCase() === filter.extension.toLowerCase());
      } else if (filter.extension.$in) {
        const exts = filter.extension.$in.map(e => e.toLowerCase());
        list = list.filter(f => exts.includes((f.extension || '').toLowerCase()));
      }
    }
